...
for i in range(traj_count):
    aver_E_vals.append(0.5/a+aver_T_vals[i]-aver_p_dot_vals[i])
...
#evaluating errors
global_aver_T_error=np.std(aver_T_vals)/math.sqrt(traj_count-1)
global_aver_p_dot_error=np.std(aver_p_dot_vals)/math.sqrt(traj_count-1)
global_aver_E_error=np.std(aver_E_vals)/math.sqrt(traj_count-1)
global_aver_kink_metr_error=np.std(kink_metr_vals)/math.sqrt(traj_count-1)
